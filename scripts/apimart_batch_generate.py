#!/usr/bin/env python3
"""APIMart batch image generation script.

Adapted from yunshu_skillshub/image-assistant for homework-coach Skill.
Reads a JSONL file of image prompts and calls APIMart API to generate images.

Usage:
    python scripts/apimart_batch_generate.py \
        --config scripts/apimart.env \
        --input "<requests.jsonl real path>" \
        --out "<output directory real path>"
"""
...
def load_requests(input_path: Path) -> list[dict[str, Any]]:
    ...
    if isinstance(obj, list) and all(isinstance(item, dict) for item in obj):
        return list(obj)
    raise ValueError("Unsupported input format: use .jsonl or a JSON array of objects")
...
def normalize_requests(
    ...
    *,
    ...
    for raw in raw_requests:
        ...
        n = raw.get("n", None)
        n_value = n if isinstance(n, int) else default_n
...
def post_json(
    *, url: str, token: str, payload: dict[str, Any], timeout_s: float
...
def download_url(
    url: str, *, timeout_s: float, ctx: ssl.SSLContext
...
def _get_json(
    url: str, *, token: str, timeout_s: float, ctx: ssl.SSLContext
) -> Any:
    req = request.Request(url=url, method="GET")
    req.add_header("Authorization", f"Bearer {token}")
    with request.urlopen(req, timeout=timeout_s, context=ctx) as resp:
...
def poll_task(
    *,
    ...
    base = parse.urlsplit(api_url)
    url = f"{base.scheme}://{base.netloc}/v1/tasks/{task_id}"
    ...
    while True:
        ...
        status = (
            str(((last_obj or {}).get("data") or {}).get("status") or "")
            .strip()
            .lower()
        )
...
def main(argv: list[str]) -> int:
    ...
    if not args.dry_run and not token:
        raise ValueError("Missing TOKEN/SK/API_KEY in config (or use --dry-run)")
    ...
    for item in normalized:
        ...
        record: dict[str, Any] = {"id": request_id, "request": item.payload}

        if args.dry_run:
            record["curl"] = render_curl_command(api_url, item.payload)
            run["items"].append(record)
            _write_run_json(run_json_path, run)
            print(f"[{request_id}] dry-run OK")
            continue

        try:
            status, headers, body = post_json(
                url=api_url, token=token, payload=item.payload, timeout_s=args.timeout
            )
        except error.HTTPError as exc:
        ...
        if response_obj is not None:
            ...
            for idx, (kind, value) in enumerate(images, start=1):
                if kind == "base64":
                    ...
                    saved_images.append(
                        {"filename": filename, "source_kind": "base64", "index": idx}
                    )
                elif kind == "url":
                    ...
                    if args.no_download:
                        continue
                    try:
                        data, mime = download_url(
                            value, timeout_s=args.timeout, ctx=ctx
                        )
                    except Exception:
                        continue
                    ext = _guess_extension_from_mime(mime)
            ...
            for task_index, task_id in enumerate(task_ids, start=1):
                ...
                for url_index, url in enumerate(task_urls, start=1):
                    try:
                        data, mime = download_url(
                            url, timeout_s=args.timeout, ctx=ctx
                        )
                    except Exception:
                        continue
                    ext = _guess_extension_from_mime(mime)
